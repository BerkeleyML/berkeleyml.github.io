...
trainset = torchvision.datasets.MNIST(root='./data', train=True,
                                        download=True, transform=transform) # torchvision has nice easy downloading 
testset = torchvision.datasets.MNIST(root='./data', train=False,            # of datasets for CV tasks like MNIST
                                       download=True, transform=transform)

def optimize(prediction_fn, loss_fn, optim, epochs, batch_size):
    acc = []
    for epoch in range(epochs):
        trainloader = torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=True) # iterable of batches
...
def train_logistic(learning_rate=0.01, epochs=50, batch_size=100):
    ...
    def softmax(x):
        exp_x = torch.exp(torch.clamp(x, max=80))
        exp_sums = torch.sum(exp_x, 1)
        return exp_x/exp_sums.view(-1, 1).expand_as(exp_x)
    
    def logistic(x):
        return softmax(torch.matmul(x, weights) + bias)
    
    def loss_fn(y, y_pred):
        return -torch.sum(y*torch.log(y_pred+1e-5))/batch_size
...
def train_nn(learning_rate=0.05, epochs=50, batch_size=50, n_hidden=64):
    ...
    def softmax(x):
        ...
        return exp_x/exp_sums.view(-1, 1).expand_as(exp_x)
    def nn(x):
        hidden = torch.tanh(torch.matmul(x, weights1) + bias1)
        return softmax(torch.matmul(hidden, weights2) + bias2)
    def loss_fn(y, y_pred):
        return -torch.sum(y*torch.log(y_pred+1e-5))/batch_size
